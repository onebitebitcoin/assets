...
const CryptoPage = () => {
  ...
  // 비트코인(암호화폐)만 필터링
  const cryptoAssets = assets.summary.assets.filter(
    (a) => a.asset_type?.toLowerCase() === "crypto"
  );
  const assetMetaById = new Map(cryptoAssets.map((asset) => [asset.id, asset]));

  const cryptoIds = new Set(cryptoAssets.map((a) => a.id));
  const filteredTableColumns = tableColumns.filter((col) => cryptoIds.has(col.id));
  ...
  const commonProps = {
    ...
    newAssetForm: { ...assets.newAssetForm, asset_type: "crypto" },
  ...
  return (
    <>
      <h2 className="page-title">비트코인</h2>
