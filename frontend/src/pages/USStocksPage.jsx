...
const USStocksPage = () => {
  ...
  // 미국 주식만 필터링
  const usStockAssets = assets.summary.assets.filter(
    (a) => a.asset_type?.toLowerCase() === "stock"
  );
  const assetMetaById = new Map(usStockAssets.map((asset) => [asset.id, asset]));

  const usStockIds = new Set(usStockAssets.map((a) => a.id));
  const filteredTableColumns = tableColumns.filter((col) => usStockIds.has(col.id));
  ...
  const commonProps = {
    ...
    newAssetForm: { ...assets.newAssetForm, asset_type: "stock" },
  ...
  return (
    <>
      <h2 className="page-title">미국 주식</h2>
